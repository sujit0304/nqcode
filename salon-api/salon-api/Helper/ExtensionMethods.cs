...
namespace salon_api.Helper
{
    ...
    {
        public static List<T> MapTo<T>(this DataTable table) where T : class, new()
        {
            var colErr = "";
            List<Tuple<object, PropertyInfo>> map = new List<Tuple<object, PropertyInfo>>();
            ...
            foreach (PropertyInfo pi in value2.PropertyType.GetProperties())
            {
                ...
                if (col == null || col.Name.IsNullOrEmpty())
                {
                    if (pi.Name.ToLower() == "id")
                    {
                        ...
                        {
                            map.Add(new Tuple<DataColumn, PropertyInfo>(table.Columns[key], pi));
                        }
                    }
                ...
                else
                {
                    ...
                    {
                        map.Add(new Tuple<DataColumn, PropertyInfo>(table.Columns[col.Name], pi));
                    }
                    else
                    {
                        map.Add(new Tuple<DataColumn, PropertyInfo>(null, pi));
                    }
                }
            }
            try
